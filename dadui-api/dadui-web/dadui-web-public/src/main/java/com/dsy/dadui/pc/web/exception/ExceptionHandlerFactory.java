package com.dsy.dadui.pc.web.exception;
...
public class ExceptionHandlerFactory {
	...
	public static ExceptionHandler createExceptionhandler(Throwable ex) {
		...
		String packageName = ExceptionHandlerFactory.class.getName()
				.replace(ExceptionHandlerFactory.class.getSimpleName(), "");
		...
		try {
			// 这里查找系统自带的,按照捕获的异常名称+
			// Handler进行查找,算是简单约定,因为框架开发中的内置我可以约定,扩展的使用配置文件进行
