...
public class CaptchaUtil {
	...
	public static void checkCaptcha(String captcha, String sessCaptcha) {
		...
		if (StringUtils.isEmpty(sessCaptcha)) {
			throw new RuntimeException("验证码未生成，验证码使用");
		}
		if (!captcha.equals(sessCaptcha)) {
			logger.warn("错误验证码 sessCaptcha:" + sessCaptcha + " captcha:" + captcha);
			throw new RuntimeException("sess:" + sessCaptcha + " param:" + captcha);
		}
	}
	...
	public static void saveSession(HttpServletRequest request, String captchaGroupId, String code) {
		...
		// System.out.println("saveSession:" + sessionKey + " captchaGroupId:" +
		// captchaGroupId + " code:" + code);
